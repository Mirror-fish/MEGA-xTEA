...
def evaluate_discordant(args, params, filenames):
    ...
    try:
        ...
        if len(for_gaussian_fitting) < 10:
            # [PHASE1_TUNABLE] Reverted to original MEGAnE behavior (skip disc
            # filtering).  The fixed threshold approach [3,5,8,15] caused
            # increased FN in insertion analysis.  Disc filtering for small
            # candidate sets will be addressed in Phase 2.
            log.logger.warning('Not enough data for Gaussian fitting of disc reads (n=%d). Will use other evidences.' % len(for_gaussian_fitting))
