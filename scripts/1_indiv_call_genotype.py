...
parser.add_argument('-i', metavar='str', type=str, help='Required. Specify an input WGS file, either BAM or CRAM format. The file name must end with either ".bam" or ".cram".', required=True)
parser.add_argument('-fa', metavar='str', type=str, help='Required. Specify reference genome which are used when input reads were mapped. E.g. GRCh38DH.fa', required=True)
parser.add_argument('-fadb', metavar='str', type=str, help='Required. Specify blastdb of reference genome. E.g. GRCh38DH.fa.db', required=True)
parser.add_argument('-mk', metavar='str', type=str, help='Optional, but highly recommended. Specify .mk file generated by the step 0 "build_kmer_set". E.g., GRCh38DH.fa.mk. Please make sure that .mi file also exists in the same directory.', required=True)
parser.add_argument('-rep', metavar='str', type=str, help='Required. Specify Repeat consensus file. E.g. Dfam_custom.ref', required=True)
parser.add_argument('-repout', metavar='str', type=str, help='Specify RepeatMasker output. Must be masked using the input RepBase file. E.g. GRCh38DH.fa.out. Not required if -repout_bed is provided.', required=False, default=None)
parser.add_argument('-repout_bed', metavar='str', type=str, help='Optional. Specify a pre-converted BED file (5 columns: chr, start, end, name:class, strand) instead of a RepeatMasker .out file. When provided, -repout is not required.', required=False, default=None)
parser.add_argument('-repremove', metavar='str', type=str, help='Required. Specify full path to a file containing the names of non-ME repeat class. E.g. non_ME_rep.txt', required=True)
parser.add_argument('-pA_ME', metavar='str', type=str, help='Required. Specify full path to a file containing repat class with polyA tail. E.g. ME_with_pA.txt', required=True)
parser.add_argument('-mainchr', metavar='str', type=str, help='Required. Specify full path if you analyze non-human sample. E.g. main_chr_list.txt', required=True)
...
parser.add_argument('-skip_unmapped', help='Optional. Specify if your sample contains many unmapped reads (which takes a long time), particularly non-human samples.', action='store_true')
...
parser.add_argument('-setting', metavar='str', type=str, help='Optional. Specify full path to the parameter setting file. Default: /path/to/MEGAnE/docs/parameter_settings.txt')
...
parser.add_argument('-L1_filt_on', action='store_true', help=argparse.SUPPRESS) # Deprecated. If on, MEGAnE applies additional filtering step for L1 insertions. This is only for compatibility with MEGAnE v0.
...
# Validate: either -repout or -repout_bed must be provided
if args.repout is None and args.repout_bed is None:
    parser.error('Either -repout (RepeatMasker .out) or -repout_bed (pre-converted BED) must be specified.')
...
filenames.mapped_fa_select0 =os.path.join(args.outdir, 'mapped_selected0.fa')
...
filenames.tmp               =os.path.join(args.outdir, 'just_tmp.txt')
filenames.tmp_header        =os.path.join(args.outdir, 'blast_tmp_header.txt')
...
if args.only_geno is False:
    ...
    if args.repout_bed is not None:
        ...
        args._repout_bed_user_provided = False
    ...
    if do_ins is True:
        ...
        if args.skip_unmapped is True:
            ...
            utils.rename_fasta_header_serial_num(filenames.unmapped_hit_fa, filenames.tmp, filenames.tmp_header)
            ...
            utils.reconstruct_blast_out_header(filenames.blast3_res, filenames.tmp, filenames.tmp_header)
            parse_blastn_result.find_chimeric_unmapped(args, params, filenames.blast3_res, filenames.unmapped_MEI)
            # del files
            utils.gzip_or_del(args, params, filenames.tmp_header)
        ...
        process_mapped_seq.remove_multimap(args, params, filenames, init.base)
    ...
    if not getattr(args, '_repout_bed_user_provided', False):
