            ...
            with open(outfilename, 'w') as outfile:
                with open(infilename) as infile:
                    for line in infile:
                        ...
                        tmp=[ls[0], ls[1], ls[2], ls[7], 'MEI_left:ref_pos=%s,chimeric=%d,hybrid=%s,pA=%s' % (l_pos, l_chim, ls[12], ls[6]), 'MEI_right:ref_pos=%s,chimeric=%d,hybrid=%s,pA=%s' % (r_pos, r_chim, ls[13], ls[5]), 'confidence:%s' % confidence_status, 'unique:%s,50bp_or_longer:%s,orig_conf:%s' % (uniq, len_status, ls[15]), 'subfamily_pred:status=%s,%s' % (pred_status, pred_res), transd_status, 'ID=%s' % ls[14]]
